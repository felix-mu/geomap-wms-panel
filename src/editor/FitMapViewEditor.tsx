...
import { InlineFieldRow, InlineField, RadioButtonGroup, Combobox } from '@grafana/ui';
...
//   labelWidth: number;
  ...
  context: StandardEditorContext<GeomapPanelOptions, any>
};
...
export const FitMapViewEditor = ({ /*labelWidth, */value, onChange, context }: Props) => {
  const layers = useMemo(() => {
    if (context.options?.layers) {
      // Only return layers with name set
      return context.options.layers.filter((el) => el.name !== undefined)
      .map((layer: SelectableValue<string>) => ({
        label: layer.name,
        value: layer.name,
  ...
  const allLayersEditorFragment = (
    <InlineFieldRow>
      <InlineField
        ...
        grow={true}
      >
        <Combobox options={layers} onChange={onSelectLayer} value={value.layer} />
      </InlineField>
    </InlineFieldRow>
  );
  ...
  return (
    <>
      <InlineFieldRow>
        <InlineField label={t('geomap.fit-map-view-editor.label-data', 'Data')} grow={true}>
          <RadioButtonGroup
